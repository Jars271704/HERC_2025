...
float x_est = 0.0; // Estimación inicial
float P_est = 1.0; // Covarianza inicial
const float Q = 0.01; // Ruido del proceso (ajustar según sea necesario)
const float R = 0.1; // Ruido de medición (ajustar según sea necesario)
...
void calcularVelocidad() {
    ...
    float velocidadMedida = rpm * (2.0 * PI / 60.0);
    ...
    x_est = x_pred + K * (velocidadMedida - x_pred);
    P_est = (1 - K) * P_pred;

    velocidadAngular = x_est; // Guardamos la estimación filtrada
}
...
void loop() {
    calcularVelocidad();  // Medir la velocidad angular con Kalman
